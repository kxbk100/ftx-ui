  ...
  return __async(this, [], function* () {
    ...
    for (let url2 of urls) {
      ...
      try {
        ...
        finishInstall(extractFileFromTarGzip(yield fetch(url2), fromPath));
        return;
      } catch (err) {
        if (!debug)
          console.error(tryText);
...
function installUsingNPM(name, file) {
  ...
  child_process.execSync(`npm install --loglevel=error --prefer-offline --no-audit --progress=false ${name}@${version}`, {cwd: installDir, stdio: "inherit", env});
